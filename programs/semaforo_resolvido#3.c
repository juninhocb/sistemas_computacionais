#include <pthread.h>
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <semaphore.h>

sem_t s;

void * th1(void *n){
	int id = *(int*)n;
	int i;
	while(1){ /* faz com que apenas DUAS threads entrem na função */ 
		sem_wait(&s);
		for(i=0;i<5;i++){
			printf("%d\n", id);
			fflush(stdout);
			sleep(1);
		} 
		sem_post(&s);
	}  /* provavelmente quem "ganhará" a corrida e acesar a seção crítica é a thread que já estava em execução */
}



int main(int argc, char **argv){
	pthread_t thread1, thread2, thread3, thread4;
	int thread_ids[4] = {0, 1, 2, 3};
	
	sem_init(&s, 0, 2);
	
	if(pthread_create(&thread1, NULL, th1, &thread_ids[0])) {
		fprintf(stderr, "Error creating thread\n");
		return 1;
	}

	if(pthread_create(&thread2, NULL, th1, &thread_ids[1])) {
		fprintf(stderr, "Error creating thread\n");
		return 1;
	}
	
	if(pthread_create(&thread3, NULL, th1, &thread_ids[2])) {
		fprintf(stderr, "Error creating thread\n");
		return 1;
	}

	if(pthread_create(&thread4, NULL, th1, &thread_ids[3])) {
		fprintf(stderr, "Error creating thread\n");
		return 1;
	}


	if(pthread_join(thread1, NULL)) {
		fprintf(stderr, "Error joining thread\n");
		return 1;
	}

	return 0;
}