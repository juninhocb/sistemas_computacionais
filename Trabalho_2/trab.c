...
continue;
            ...
            //	printf("\n \n VALOR DO CLUSTER: %ld \n \n", sizeof(dir_pos));
            unsigned int cluster = dt->entry[i].DIR_FstClusHI<<16 | dt->entry[i].DIR_FstClusLO; 
            
			
            dir_pos =  (FAT32_vol.BPB_RsvdSecCnt + (FAT32_vol.BPB_NumFATs * FAT32_vol.BPB_FATSz32) + (cluster - 2)) * FAT32_vol.BPB_BytsPerSec;
            read_data(dev, dir_pos, &dt, 512);
	
            print_dir(&dt); //chama recursivamente
